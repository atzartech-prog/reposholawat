const sholawatData = [];�ُSْنُ الْخَوَاتِمِ وَيُسْتَسْقَى الْغَمَامُ بِوَجْهِهِ الْكَرِيمِ",
...
async function init() {
  ...
  try {
    const response = await fetch('sholawat_db.json');
    if (!response.ok) throw new Error("Gagal mengambil data sholawat.");
    const data = await response.json();
    sholawatData.length = 0;
    sholawatData.push(...data);
  } catch (err) {
    console.error("Error loading sholawat database:", err);
    showToast("Gagal memuat basis data sholawat. Menggunakan lirik bawaan.");
  }
